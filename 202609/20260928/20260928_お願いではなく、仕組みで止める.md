...
具体的には、実行前フックを用いて `git push --force` を確実に阻止する設定例と、その動作を安全に検証する手順を解説します。  
なお、コード整形やログ収集といった「実行後フック」については、今回は対象外としています。  

:::caution
本記事は2026年9月24日時点の公式ドキュメントをもとに作成しています。
確認できた動作と、各自の環境で検証が必要な箇所については、分けて記載しています。
:::
...
ただし、スクリプトの設定ミスや処理のタイムアウトなどが発生した場合は、当然ながら止まらないこともある点には注意が必要です。  
...
- 取り消しの効かないリモート操作：`git push --force`、リモートブランチの削除
...
GitHubの公式ドキュメントでは `preToolUse` と先頭小文字で表記されていますが、`PreToolUse` と先頭大文字で指定することも出来ます。
先頭大文字で指定した場合、Claude Code互換モードとして認識され、Claude Code形式の `matcher` やツール名、入力プロパティ名（`tool_input` 等）がそのまま利用できるようになります。  
...
なお、ローカルとCopilotのいずれのモードでも `.github/hooks/*.json` が探索されます。そのため、上記の設定とローカル用の設定を同一リポジトリ内に併せて配置してしまうと、意図しないフックまで同時に実行されてしまう原因になります。動作検証の際は、まずは別々のテスト用ディレクトリを用意して試すのが確実です。  
...
したがって、重大なインシデントにつながる操作を確実に阻止したい場合は、hooks単体だけに頼るのではなく、以下のような多層的な対策と組み合わせることが推奨されます。  
...
一方、Copilot（Session Target = Copilot）では異常終了時に停止する挙動となります。環境差を避けるためにも、各クライアントに適したJSON形式で `deny` を返す実装にしておくのが確実です。
- タイムアウトの発生：  
...
これは裏を返せば、第三者のリポジトリをクローンして不用意にエージェントを起動した場合、**そのリポジトリに含まれる任意のスクリプトがローカルPC上でそのまま実行されてしまうリスクがある**ことを意味します。  
...
そうした丁寧なひと手間をかけることで、AIエージェントに安心して任せられる領域を確実に広げていくことができます。
